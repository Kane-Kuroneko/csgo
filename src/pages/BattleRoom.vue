<template>
	<div class = "roll-content">
		<div class = "common-bg">
			<div
				class = "margin-auto"
				style = "min-height: 4rem; position: relative"
			>
				<div
					class = "ranking-bg"
					:class = "{ 'reverse-bg': roomData.type === 1 }"
				>
					<div>
						<Title name = "箱子对战"></Title>
						<div
							class = "room-state-battle-over"
					...
					<div
						class = "back-room cursor hide-m"
						@click = "$router.push('/page/caseBattle')"
					>
						<div class = "battle-room-tap">
							...
							>
								<img
									src = "../assets/image/battle/goback.png"
									width = "100%"
									alt = ""
								/>
							</div>
					...
					<div class = "room-url">
						<div class = "promotion-code">
							...
							<span style = "opacity: 0.5">{{ roomUrl }}</span>
							<div
								@click = "clipboard(roomUrl)"
								class = "default-btn copy-btn"
							>复制
							</div>
						</div>
					</div>
					<div class = "room-state-info">
						...
						>
							<ul class = "room-style">
								<li
									@click = "boxGoods(item.boxId, item)"
									class = "box-lst"
									:class = "[
										{ 'box-lst-over': roomData.status !== '2' && round > idx },
										{ 'box-lst-over': roomData.status === '4' && round > idx },
										{ cursor: roomData.status !== 4 },
									]"
									align = "center"
									v-for = "(item, idx) in roomData.boxMap"
									:key = "idx"
								>
					...
					>
						<img
							class = "cursor war-hover"
							src = "../assets/image/battle/verifyhash.png"
							alt = ""
							@click = "$refs.userVertify.getVerify()"
						/>
					</div>
					<div
						class = "countdown-time hide-m"
						:style = "{ visibility: roomData.status == '2' ? 'unset' : 'hidden' }"
					>
						<div
							@click = "reveseTips = true"
							class = "cursor game-tips hide-m"
						...
						<div>
							<div class = "minutes-countdown">
								{{ Math.floor(roomData.time / 60) > 0 ? Math.floor(roomData.time / 60) : 0 }}
							</div>
							<div class = "countdown-split">：</div>
							<div
								class = "minutes-countdown"
								style = "margin-right: 3px"
							>{{ countDown(roomData.time , 1) }}
							</div>
							<div class = "minutes-countdown">
						...
						<div
							...
							:style = "{
								visibility:
									roomData.createBy === user.id && roomData.status === '2' && roomData.roomType !== 2
										? 'unset'
										: 'hidden',
							}"
						>
							<div
								class = "invite-conuntdown"
								v-if = "roomData.invitationTime > 0"
							>
								<i
									class = "iconfont iconicon-password"
									style = "margin-right: 5px; transform: translateY(-2px)"
								></i
								>
								在
								<div style = "display: flex; margin: 0 3px">
									...
									<div class = "minutes-countdown">{{ countTime(roomData.invitationTime , "seconds") }}</div>
								</div>
								内仅限被邀请者加入
							</div>
							<div
								class = "default-btn battle-btn"
								@click.stop = "
									$refs.inviteUser.isShow = true;
									$refs.inviteUser.hoster = roomData.createBy;
								"
							>
					...
					<div
						class = "countdown-time-m hide-p hide-ipad"
						v-show = "roomData.status == '2'"
					>
						<div style = "display: flex">
							...
							<div
								...
								style = "margin-right: 3px"
							>
								{{ countTime(roomData.time , "senond") }}
							</div>
							<div class = "minutes-countdown">{{ countTime(roomData.time , "seconds") }}</div>
						</div>
					</div>
					<div
						class = "unstart-state hide-p hide-ipad"
					...
					>
						<div
							...
							v-if = "roomData.invitationTime > 0"
						>
							<i class = "iconfont iconicon-password"></i>
							在
							<div style = "display: flex; margin: 0 3px">
								<div class = "minutes-countdown">{{ countTime(roomData.invitationTime , "minutes") }}</div>
								<div>:</div>
				...
				<div class = "waiting-box">
					<ul class = "player-container">
						<li
							class = "player-state"
							:class = "'player-state-' + roomData.userMap.length"
							v-for = "(item, idx) in roomData.userMap"
							:key = "idx"
						>
							<!-- 头像 -->
							<div
								class = "player-avator-container"
								:class = "[
									roomData.status != '2' ? (item.result === 1 ? 'avator-win' : 'avator-loser') : '',
									{ 'play-rabit': item.userType == 1 },
								]"
							>
								<SkinsAvatar
									wd = "100px"
								...
								<img
									...
									src = "../assets/image/battle/crown.png"
								/>
							...
							<div
								class = "player-name"
								:class = "{ 're-name': roomData.type === 1 }"
							>
								<p>
									<img
										src = "../assets/image/battle/rabit.png"
							...
							<div
								class = "playing-state"
								align = "center"
								:class = "{ 'play-rabit': item.userType == 1 }"
							>
								<div
									style = "height: 100px"
									v-show = "roomData.status == '2'"
								>
									<!--    已加入对战-->
									<p v-show = "item.btnJurisdiction === 1">
										<i class = "iconfont iconicon-select"></i>
										已加入对战
									</p>
									<div v-show = "item.btnJurisdiction !== 1">
										...
										>
											<div
												class = "invite-conuntdown no-robot"
										...
										<!--    房主添加机器人-->
										<div
											v-show = "[22, 55].includes(item.btnJurisdiction)"
											v-if = "roomData.roomType !== 2"
										>
											<div
												class = "no-robot"
												v-show = "roomData.robotTime > 0"
											>
												<i class = "iconfont iconicon-tips"></i>
												{{ roomData.robotTime }}s内不可加入机器人
											</div>
											<div
												v-show = "roomData.robotTime < 0 || !roomData.robotTime"
												class = "unstart-state"
											>
												<div
													class = "default-btn battle-btn"
													@click.stop = "isRabotJoin = true"
												>
													<i class = "iconfont iconicon-add"></i>
													添加对战机器人
												</div>
										...
										<div
											class = "no-robot"
											v-show = "[77].includes(item.btnJurisdiction)"
										>
											<i class = "iconfont iconicon-tips"></i>
											反转模式不可添加机器人
										</div>
								...
								>
									<div class = "open-box-battle">
										<div ref = "pool">
											...
											>
												<li
													v-for = "(el, idx) in item.openBoxData.prize.prizePool"
													:key = "idx"
								...
								<div v-if = "roomData.status == '4'">
									...
									>
										<div>
											<img
												src = "../assets/image/battle/winbg.png"
												alt = ""
												v-if = "item.result == 1"
											/>
											<img
												src = "../assets/image/battle/losebg.png"
												alt = ""
												v-else
											/>
										</div>
									...
									<div
										class = "winned-money"
										:class = "{ 'winned-loser': item.result !== 1 }"
									>
										<div class = "line-img">
											<div></div>
										...
										<p>
											<i class = "iconfont icon-icon_diamond1"></i>
											{{ item.result == 1 ? item.resultAmount : item.resultMap[0].price }}
										</p>
										<div class = "line-img">
											<img
												src = "../assets/image/battle/endingState.png"
							...
							<div class = "player-pool">
								...
								>
									<img
										src = "../assets/image/battle/jioned.png"
										v-show = "item.userImg"
									/>
									<img
										src = "../assets/image/battle/unjion.png"
										v-show = "!item.userImg"
									/>
								</div>
								<!--                                等待状态 已加/代加-->
								...
								>
									<div>
										...
										>
											<div>
												<i class = "iconfont iconicon-ready"></i>
											</div>
											<p>准备游戏</p>
										</div>
									</div>
								...
								<!--                                开始物品池-->
								<div
									v-if = "roomData.status == '3' || roomData.status == 4"
									class = "battle-prize-pool"
								>
									<skins-item
										:skins-data = "item.prizePool"
		...
		>
			<div></div>
			<img
				src = "../assets/image/battle/wating.png"
				alt = ""
			/>
		...
		<hash-module
			...
			width = "6"
		>
			<p class = "modal-content">是否添加对战机器人</p>
			<div
...
export default {
	...
	created () {
		...
		this.timer = setInterval(() => {
			...
			if ( data.time < 1 && data.status === "2" ) {
				if ( + data.isFastBattle === 0 ) {
					// 普通房人未满
					this.closeRoom();
				} else if ( + data.isFastBattle === 1 && !data.userMap[1].userId ) {
					// 快速房无人参加
					this.closeRoom();
				}
	...
	methods : {
		...
		countTime (time , type) {
			...
			let senond = Math.floor(time % 60).toString().length === 2 ? Math.floor(time % 60).toString().split("")[0] : 0;
			let seconds = Math.floor(time % 60).toString().length === 2 ? Math.floor(time % 60).toString().split("")[1] : Math.floor(time % 60);
			let arr = {
				minutes ,
				senond ,
				seconds ,
		...
		handleBtnJurisdiction (el) {
			...
			if ( this.roomData.type === 0 && !el.userImg ) {
				...
				if ( id !== roomHost ) {
					...
					if ( this.roomData.invitationTime > 0 ) {
						// 邀请者模式
		...
		async initOpen (round) {
			return new Promise(resolve => {
				...
				for ( let supplement = 0 ; supplement < prizePoolCount - this.artical[round].length ; supplement ++ ) {
					//补充奖池
		...
		setPrize (prizes) {
			...
			setTimeout(() => {
				...
				if ( this.round < this.roomData.boxMap.length + 1 ) {
					this.webSocket.send(this.round);
