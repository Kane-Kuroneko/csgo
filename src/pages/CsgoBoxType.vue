<template>
  <div class="csgo-box">
    <div class="csgo-content" :style="{'overflow': delayTime  ?  'hidden' : 'unset'}">
      <!--      饰品详情-->
      <div class="result-name" v-show="resultSkins.stuffName !== ''">
        <div> {{ resultSkins.stuffName }}</div>
        <div class="bottom-color"></div>
      </div>
      <div class="opening-start" v-show="resultSkins.stuffName === ''" style="height: 100%">
        <div class="box-info">
          <p class="title-one">开箱</p>
          <p class="title-two">{{ box.name }}</p>
        </div>
        <div class="box-pos">
          <div :style="aniStyle" class="ani-csgo" ref="boxAni">
            <img src="../assets/image/csgo/csgobox.png" v-show="delayTime" alt="">
            <img src="../assets/image/csgo/boxMove.gif" v-show="!delayTime && !aniOpening" alt="">
          </div>
        </div>
        <div class="box-pos" style="position: absolute;width: 90%;top:117px;">
          <div class="box-ani" align="center">
            <img src="../assets/image/csgo/boxopen.png" v-show="aniOpening" alt="">
          </div>
        </div>
        <div v-show="boxAni || resultSkins.stuffName !== ''" style="height: calc(100% - 50vh )">
          <div v-show="resultSkins.stuffName === ''" style="display: flex;flex-direction: column;height: 100%">
            <p class="box-tips">箱内饰品：</p>
            <div class="split-line"></div>
            <div class="skins-content">
              <ul>
                <li align="center" v-for="el in skinsImg">
                  <div class="skins-img " :class=" 'border-csgo-' +conversionStyle(el.rarity , el.category)">
                    <img :src="imgHost(el.image)" alt="" style="width: 90%">
                  </div>
                  <p class="name-one over-flow-hide" :title="el.name1">{{ el.name1 }}</p>
                  <p class="name-two over-flow-hide" :title="el.name2">{{ el.name2 }}</p>
                </li>
              </ul>
            </div>
          </div>
      ...
      <!--      开箱动画-->
      <div class="opening-ing" v-show="aniOpening && resultSkins.stuffName === ''">
        <div class="line"></div>
        <div class="scale-circle-bg zoom-index" ref="pool">
          <div :style="openingAniZoom" ref="prizePool">
            <div class="ani-item" v-for="(el,idx) in prizePool"
                 :ref="'item-' +idx"
                 :key="idx"
                 :class="idx+'-item'"
                 :style="{'background':`url(${require(`../assets/image/csgo/${conversionStyle(el.rarity,el.category)}.png`)}) no-repeat top left / 100%`}">
              <img :src="el.image" alt="" style="width: 90%">
            </div>
          </div>
        </div>
      ...
      <!--      结果展示-->
      <div class="opening-end" v-show="resultSkins.stuffName !== ''">
        <div class="official-result">
          <img :src="imgHost(resultSkins.stuffImage)" alt="">
        </div>
        <div class="circle-circle"></div>
      </div>
      <div class="circle-circle" v-show="!aniOpening && !boxAni"></div>
    </div>
    <div class="button-content" v-show="boxAni || resultSkins.stuffName !== '' "
         :class="{'result-button': resultSkins.stuffName !== ''}">
      <div class="lf-content">
        <div class="open-button" @click="openBox"><i class="iconfont icon-icon_diamond1 font-s"></i> {{
          box.price*$parent.boxDiscount.discount }}
        </div>
        <div class="game-tips-words" style="margin: 0 20px 0  0"> 本站所有游戏道具，只能本人取回到游戏内使用，均不允许站内交易及互换</div>
      </div>
      <div class="rt-content" @click="quitCsgo">
        关闭
      </div>
    </div>
  </div>
    ...
    methods: {
      ...
      openBox() {
        ...
        this.checkOpen().then(() => {
          ...
          setTimeout(() => {
            ...
            timer = setInterval(() => {
              try {
                console.log(this.$refs.pool.scrollWidth)
      ...
      checkOpen() {
        return new Promise((resolve, reject) => {
          if (this.skinsImg.length === 0) reject({msg: '空箱,勿点', type: 1})
          if (!localStorage.getItem('token')) reject({msg: '未登录', type: 2})
          if ((this.box.discount * this.box.price).toFixed(2) > this.$parent.user.balance) reject({
            msg: '余额不足',
            type: 3
          })
          this.initState()
...
<style scoped lang="less">
  ...
  .mask{
    ...
    height: 215px;
    z-index: 100;
    background: linear-gradient(90deg, rgba(255, 255, 255, .2) 1%, transparent 10%, transparent 90%, rgba(255, 255, 255, .2) 100%);
  }
  ...
  .zoom-index{
    mask-image: radial-gradient(circle closest-side, #fff0 70%, #000 70%);
  ...
  .circle-circle{
    ...
    aspect-ratio: 1;
    border-radius: 50%;
    border: 200px solid rgba(37, 39, 50, .5);
    z-index: 3;
  }
