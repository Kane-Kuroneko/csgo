<template>
	<div class = "bg">
					...
					>
						...
						<div class = "right">
							...
							<div class = "right-middle">
								...
								<img
									src = "../assets/image/Welfare/flag.png"
									alt = ""
									v-if = "ite.flag"
								/>
							</div>
							<div class = "progress-div">
								<div class = "unfinished">
									<div
										:style = "{
											width: Math.floor((ite.amount / ite.codeMap) * 100) + '%',
											background:
												Math.floor((ite.amount / ite.codeMap) * 100) >= 100
													? 'rgba(42, 252, 1, 1)'
													: '#ffffff',
										}"
									></div>
									<div
										:style = "{
											width: 'calc(' + (100 % -(Math.floor((ite.amount / ite.codeMap) * 100) + '%')) + ')',
										}"
									></div>
								</div>
							</div>
		...
		<!-- 左侧图片 -->
		<img
			src = "../assets/image/Welfare/left.png"
			alt = ""
			class = "left-position hide-m"
		/>
		<!-- 固定icon -->
		<div class = "right-position hide-m">
			<a
				@click.prevent = "Md('maodian0')"
				v-show = "list[0].data.length > 0"
			>
				<img
					src = "../assets/image/Welfare/day-icon.png"
					alt = ""
				/>
			</a>
			<a
				@click.prevent = "Md('maodian1')"
				v-show = "list[1].data.length > 0"
			>
				<img
					src = "../assets/image/Welfare/week-icon.png"
					alt = ""
				/>
			</a>
			<a
				@click.prevent = "Md('maodian2')"
				v-show = "list[2].data.length > 0"
			>
				<img
					src = "../assets/image/Welfare/month-icon.png"
					alt = ""
				/>
			</a>
		</div>
			...
			list : [
				{
					...
					titleTip : "当日00:00至次日00:00期间数据满足开箱条件时开启，每日00:00重置" ,
					data : [] ,
				} , {
					titleImg : require("../assets/image/Welfare/week.png") ,
					titleTip : "周一00:00至次周周一00:00期间数据满足开箱条件时开启，每周一00:00重置" ,
					
					data : [] ,
				} , {
					titleImg : require("../assets/image/Welfare/month.png") ,
					titleTip : "当月1日00:00至次月1日00:00期间数据满足开箱条件时开启，每月1日00:00重置" ,
...
.info {
	...
	.title-tip {
		...
		& > div:nth-child(1) {
			...
			height: 2px;
			opacity: 0.3;
			background: linear-gradient(to right, rgba(145, 104, 104, 0), rgba(204, 204, 204, 0.5));
		}
		...
		& > div:nth-child(3) {
			...
			height: 2px;
			opacity: 0.3;
			background: linear-gradient(to left, rgba(145, 104, 104, 0), rgba(204, 204, 204, 0.5));
		}
	}
