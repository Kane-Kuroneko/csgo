		...
		<div class = "skins-container">
			<div class = "home-title">
				<div>
					<div class = "rotate-img">
						<img
							src = "../assets/image/Welfare/titleLine.png"
							alt = ""
						/>
					</div>
				</div>
			...
			<div class = "skins-detaile layout">
				<div
					v-for = "el in artical.stuffVOList"
					:key = "el.id"
					:class = "'border-top-' + el.random"
				>
					<div
						class = "skins-img"
						:class = "'drop-qua-' + el.random"
						align = "center"
					>
						<div>
							<p>${{ el.price }}</p>
				...
				>
					...
					>
						...
						>
							...
							<div
								class = "sell-name"
								:class = "{ 'special-prize': box.discountPrice.toFixed(2) < el.price }"
							>
								<p>
									<i
										class = "iconfont iconicon-bg"
										:class = "'drop-gun-' + conversionStyle(el.skinsRarity)"
									></i
									>
									{{ el.stuffName.split("|")[0] }}
								</p>
					...
					>
						<div>
							...
							<div
								...
								@click = "sellAllOrInstorage('all')"
							>
								全部出售 ${{ totalPrice.toFixed(2) }}
							</div>
						</div>
					</div>
					<div
...
export default {
	...
	computed : {
		...mapState("account" , ["activityRechargeAmount"]) , ...mapState("account" , {
	...
	methods : {
		...
		openBox () {
			...
			this.checkOpen().then(async () => {
				...
				if ( e.type === 1 ) this.$Message.info(e.msg); else if ( e.type === 2 ) this.$login(); else if ( e.type === 3 ) this.$Modale("余额不足" , "<p>立即充值</p>" , 1);
				this.openLoading = false;
				this.$emit("boxstatus" , this.openLoading);
			});
		} ,
		checkOpen () {
			return new Promise((resolve , reject) => {
				if ( this.artical.stuffVOList.length === 0 ) {
					reject({
						msg : "空箱,勿点" ,
						type : 1 ,
					});
		...
		initOpen (count) {
			return new Promise(resolve => {
				...
				while ( openBoxData.length < count ) {
					...
					for ( let supplement = 0 ; supplement < prizePoolCount - this.artical.stuffVOList.length ; supplement ++ ) {
						//补充奖池
